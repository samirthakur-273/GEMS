...
class FriendFamilyOtpPage extends StatefulWidget {
  final token;
  final otp;
  final userType;
  final id;
  ...
  const FriendFamilyOtpPage(
      ...
      this.id,
      this.email,
      this.mobilenumber, this.membershipNo})
      : super(key: key);

  @override
  _FriendFamilyOtpPageState createState() => _FriendFamilyOtpPageState();
}

class _FriendFamilyOtpPageState extends State<FriendFamilyOtpPage>
  ...
  void response(VerifyOtpModal verifyOtpModal) {
    setState(() async {
      if (verifyOtpModal.status == true) {
        ...
        GemsGLobals.userId = verifyOtpModal.values!.gemsCustomerId ?? '';
        ...
        GemsGLobals.referralRelationType=verifyOtpModal.values!.relationType.toString().toLowerCase();
        makesenseEventLoginSuccessCall();
        Navigator.push(
            context,
            MaterialPageRoute(
